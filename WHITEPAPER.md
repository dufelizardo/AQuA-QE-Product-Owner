...
> Agente de engenharia de requisitos que gera User Stories, Épicos e Critérios de Aceitação a partir de PRDs (gerados pelo agente irmão AQuA-QE Product Manager), documentos de requisitos, tickets Jira e páginas Confluence — com rastreabilidade obrigatória à fonte, validação automática e revisão humana no centro do ciclo.
...
O AQuA-QE Product Owner é um agente que automatiza essa estruturação sem remover a decisão humana do processo. A partir de uma fonte de requisitos (um PRD gerado pelo agente irmão AQuA-QE Product Manager, um arquivo `.txt`/Markdown, texto de chat, ticket Jira ou página Confluence), extrai requisitos candidatos, identifica ator/objetivo/regras de negócio, gera o artefato (User Story ou Épico completo) no formato ágil padrão, valida automaticamente contra INVEST/DoR, submete a um segundo LLM como revisor independente e só então apresenta o resultado para aprovação humana explícita. Nenhuma saída é aprovada pelo próprio agente. O PO nunca gera ou edita um PRD — essa responsabilidade é exclusiva do Product Manager.
...
- `validate_story`, `validate_epic` — checklist INVEST/DoR.
...
- `retrieve_chunks` — indexa e busca em `knowledge/methodology/` sob demanda (collection `knowledge_methodology`).
- `record_refinement_answer`/`suggest_refinement_answer` — memória institucional de respostas humanas de ciclos de refinamento (collection separada `refinement_answer_memory`): grava cada resposta dada num ciclo (Story ou Epic) e sugere — nunca aplica automaticamente — a mais parecida já dada antes, para uma pergunta semelhante num ciclo futuro do mesmo ou de outro projeto (ver seção 11).

Skills de I/O externo:

- `read_text_file` (disco), `read_jira_issue`/`read_confluence_page` (leitura, Jira/Confluence Cloud REST API), `export_markdown` (disco), `update_jira_issue`/`create_jira_epic`/`update_jira_epic`/`create_jira_story` (escrita, Jira Cloud REST API — **só executadas após aceitação humana explícita no CLI, nunca automaticamente**). Escrita/publicação no Confluence não existe mais neste agente (era só para o PRD que o próprio PO gerava — ver seção 1); `read_confluence_page` continua sendo leitura apenas.
...
- **Confluence Cloud** (REST API v1) — apenas leitura de página (`read_confluence_page`), convertendo o storage format (XHTML) para texto puro via `html.parser.HTMLParser` da stdlib (sem dependência nova), usada como fonte de entrada para o modo lote (`--confluence`). Criação/atualização de página não existem mais neste agente — eram só para publicar o PRD que o próprio PO gerava; publicação de PRD é responsabilidade exclusiva do AQuA-QE Product Manager agora.
...
- **Memória de projeto/longo prazo** (`memory.md`) — distinta do RAG sobre `knowledge/methodology/`. Uma frente já foi implementada: memória institucional de respostas de ciclos de refinamento (`record_refinement_answer`/`suggest_refinement_answer`, seção 5), via uma segunda collection Qdrant. As demais frentes (preferências de formato do PO humano, glossário consolidado entre sessões) continuam não implementadas.
...
Ver `README.md`/`README.pt.md` para o passo a passo completo de instalação (Python 3.12+, `uv`, Ollama + modelos, `.env.example` → `.env`) e todos os exemplos de uso do `run.py` (`--modo unitario`/`lote`, `--arquivo`/`--texto`/`--jira`/`--confluence`, `--refinar`, `--criar-jira`, `--priorizar`, `--saida-rtm`).
