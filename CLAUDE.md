...
- **Piloto de provedor via toggle** (`LLM_PROVIDER`, opcional — padrão continua `ollama`): `llm_service.py` também suporta `nvidia` (NVIDIA NIM), `cerebras` (Cerebras Inference) e `google` (Google AI Studio), todos via SDK `openai` contra endpoint compatível. `generator_model()`/`reviewer_model()` resolvem o par de modelos ativo; `complete()`/`complete_json()` mantêm assinatura pública inalterada. Modelos padrão por provedor (validados ao vivo nos agentes irmãos AQuA-QE Product Manager/Solution Architect antes de portar para cá): NVIDIA `deepseek-ai/deepseek-v4-pro`/`meta/llama-3.3-70b-instruct`, Cerebras `gpt-oss-120b`/`zai-glm-4.7`, Google `gemini-3.1-flash-lite`/`gemini-2.5-flash-lite`. Google usa `max_tokens=8192` fixo (default da API trunca respostas ricas) e `complete_json` usa `json.JSONDecoder().raw_decode` (tolera lixo após o JSON válido, achado ao vivo no Gemini). **Não afeta embeddings** — `embedding_service.py`/`bge-m3` continuam sempre Ollama, sem toggle. Ver `.env.example` para as variáveis por provedor.
